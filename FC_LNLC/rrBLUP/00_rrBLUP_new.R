...
rrBLUP <- function(xx,name,pheno){
  ...
  for (n in 1:ncol(pheno)){
    ...
    write.table(corallall,paste("rrBLUP/pearson_",name,"_trait_",n,".csv",sep=""),sep=",",row.names=F,col.names=F)
}
}

rrBLUP(geno,"LN_within",datafcln)
